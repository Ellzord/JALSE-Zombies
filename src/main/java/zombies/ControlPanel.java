...
public class ControlPanel extends JPanel {
	...
	ControlButton(final String label) {
	    super(label);
	...
	ControlChangeListener(final Method method) {
	    changeMethod = method;
	}
	...
	public void stateChanged(final ChangeEvent event) {
		...
		try {
		    changeMethod.invoke(zombiesPanel, new Object[] { (int) source.getValue() });
	...
	ControlLabel(final String label) {
	...
	ControlSlider(final int min, final int max, final int ticks, final int initial, final String methodName) {
	    ...
	    Method method;
	    try {
		method = ZombiesPanel.class.getMethod(methodName, Integer.class);
    ...
    private final ZombiesPanel zombiesPanel;
	...
	add(new ControlSlider(50, 200, 50,
	...
	add(new ControlSlider(0, 200, 50, PersonProperties.Healthy.SIGHT_RANGE, "setHealthySightRange"));
	add(Box.createVerticalGlue());

	add(new ControlLabel("Infected Sight Range"));
	add(new ControlSlider(0, 200, 50, PersonProperties.Infected.SIGHT_RANGE, "setInfectedSightRange"));
	add(Box.createVerticalGlue());

	add(new ControlLabel("Infection Time (s.)"));
	add(new ControlSlider(0, 30, 5, (int) PersonProperties.INFECTION_TIME_SECONDS, "setInfectionTime"));
	add(Box.createVerticalGlue());

	add(new ControlLabel("Starvation Time (s.)"));
	add(new ControlSlider(0, 30, 5, (int) PersonProperties.STARVE_TIME_SECONDS, "setStarveTime"));
	add(Box.createVerticalGlue());
