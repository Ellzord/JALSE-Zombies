...
public class Infect implements EntityTypeListener {
    ...
    private static List<Class<? extends Entity>> types = Arrays.asList(Healthy.class, Carrier.class, Infected.class,
	    Corpse.class);
	...
	for (final Class<? extends Entity> t : types) {
	...
	try {
	    final Class<? extends PersonProperties.Properties> properties = PersonProperties.getPropertiesForType(type);
	    person.setColor((Color) properties.getDeclaredField("COLOR").get(null));
	    person.setSpeed((double) properties.getDeclaredField("SPEED").get(null));
	    person.setSightRange((int) properties.getDeclaredField("SIGHT_RANGE").get(null));
	} catch (IllegalArgumentException | IllegalAccessException | NoSuchFieldException | SecurityException e1) {
	    System.err.println("Problem getting fields for type " + String.valueOf(type));
	}
	...
	try {
	    person.setDirectionMethod(MovePeople.class.getMethod(directionMethod, Person.class, Stream.class));
	} catch (final NoSuchMethodException e) {
	    System.err.println("Method " + directionMethod + " is undefined for Action MovePeople.");
	} catch (final SecurityException e) {
	    System.err.println("Method " + directionMethod + " is not public for Action MovePeople.");
	}
    }
}